...
<div bind:this={scrollContainer} class="flex-1 overflow-y-auto p-4 bg-[#8CABD9]" role="log" aria-label="チャットメッセージ">
	<ul class="space-y-3 list-none m-0 p-0">
		{#each messages as msg (msg.id)}
			{@const isMine = msg.participantId === currentParticipantId}
			<li>
				<article class="flex" class:flex-row-reverse={isMine}>
					{#if !isMine}
					...
					<div class="max-w-[75%]" class:ml-10={isMine}>
						{#if !isMine}
							<p class="text-xs text-white/90 mb-0.5 ml-1"><strong>{msg.nickname}</strong></p>
						{/if}
